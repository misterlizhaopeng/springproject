...
aop 实现种类：
    实现aop的技术不仅仅只有spring的实现，其他技术也存在，对于spring实现的aop技术有多种，当前@AspectJ方式是最常用的，
    其他的还有【AspectJ 注入切面】、【XML配置AOP】、【使用ProxyFactoryBean和对应的接口实现AOP】
...
aop 相关概念：
    ...
    通知：该切面中的方法
    [连接点]：[被切的方法]
    切点：@Pointcut，意思是被切对象的条件是什么，由切点表达式表示
